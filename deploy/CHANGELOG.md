...
- 变更内容：部署不再默认写入 `NACOS_REGISTER_IP=10.66.0.2`。未显式配置 GitHub variable 时从 `.env` 删除该项，服务自动探测容器网卡 IP，供同机 Gateway 访问。若设置了 `NACOS_REGISTER_IP`，`deploy.sh` 会从 Gateway 探测该地址:5030。
- 机器侧操作：生产 `.env` 已删除 `NACOS_REGISTER_IP` 并重建容器。跨主机 `10.66.0.2:5030` 仍受 Docker Desktop 对 WireGuard 网卡端口映射限制。
- 验证命令：Gateway `GET /api/finance/health` 返回业务 `status=UP`。
...
- 变更内容：部署时强制维护 `NACOS_REGISTER_IP`，默认使用 `10.66.0.2`；Finance 容器发布 `5030` 到宿主机，使云端和本地 Gateway 都能访问 Nacos 注册实例。
- 机器侧操作：部署后确认 Nacos 中 Finance 实例为 `10.66.0.2:5030`，并以管理员运行 Gateway 仓库的 WireGuard 防火墙脚本。
- 验证命令：检查 `docker ps` 端口、`Test-NetConnection 10.66.0.2 -Port 5030` 和 Gateway `/api/finance/health`。
...
- 机器侧操作：更新部署目录 `.env` 的 `NACOS_SERVER`，重建 Finance 容器并确认健康检查通过。
